...
public class Effect {
	 ...
	 *
	 *  tint: 255 + (src - 255)*amount = 255(1-amount) + src*amount
	 * 	shade: 0 + (src-0)*amount = src*amount
	 *
	 * <pre>
	 * new_R = dst_R + (src_R - dst_R) * amount = dst(1-amount) + src(amount)
	...
	public static Bitmap tone(Bitmap bitmap, @ColorInt int color, float amount) {
		...
		if(NATIVE) {
			...
			// RGB scale, amount doesn't effect on alpha channel.
	 ...
	 * B' = B * (1 - shade_factor)
	 * </pre>
	 * told formula is like filtering current bitmap with blackColor * amount.
	 * 
	 * @param bitmap
	 * @param amount
	 */
...
//		Bitmap result = bitmap.copy(Bitmap.Config.ARGB_8888, true);
	...
	// انتخاب کردن ناحیه مجاور با رنگ خاص و لایه خاص.
