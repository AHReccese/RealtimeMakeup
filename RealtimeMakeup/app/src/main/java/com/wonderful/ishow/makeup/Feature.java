...
public class Feature {
    ...
    static {
        // todo cannot find declaration to go.
    ...
    private static String loadClassifier(Context context) {
        ...
        // better to run on worker Thread
    ...
    public static PointF[] detectFace(Context context, Bitmap image, @Nullable String image_name) {
        ...
        long difference = afterLoad - startTime;
        Log.v("Time", "Loading Classifier: " + difference);
        PointF[] pointFS = nativeDetectFace(image, image_name, CLASSIFIER_DIR); // 200-300 ms
        long afterDetection = System.currentTimeMillis();
        Log.v("Time", "FaceDetection: " + (afterDetection - afterLoad));
        return pointFS;
