...
{
	uint64 begin = GetTime();


	// todo add benchmark -> vector or Array.
	// todo decrease function call
	// todo change function to 1-contextual or 2-inline mode.(for all functions)
	// todo avoid passing Landmark points -> save in cpp.
	// (realtime)todo avoid creating new landmark points.
	...
	for(; i < 13; ++i)
		// todo change to : points[i] = Point2f(landmarks[i*2], landmarks[i*2+1]);
		points.push_back(Point2f(landmarks[i*2], landmarks[i*2+1]));
...
//	cv::circle(image, Point(cvRound(eye_cheek_3l.x), cvRound(eye_cheek_3l.y)), 1, CV_RGB(0, 255, 0), 1, LINE_AA);
	...
	__android_log_print(ANDROID_LOG_VERBOSE, "NativeTime", "ProcessLandmarks : %lu",
						time_spent);
...
std::vector<cv::Point2f> Feature::detectFace(const cv::Mat& image, const std::string& tag, const std::string& classifier_dir)
{
	...
	uint64 end = GetTime();
	uint64 time_spent = end - begin;
	__android_log_print(ANDROID_LOG_VERBOSE, "NativeTime", "NativeFaceDetection : %lu",
						time_spent);
