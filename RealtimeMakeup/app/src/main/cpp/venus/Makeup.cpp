...
//	std::cout << __FUNCTION__ << " angle: " << rad2deg(angle) << '\n';
        ...
        Mat rightMat; // = Mat();
        Mat* rightMatPtr = &rightMat;

        Point2f originRightBrowPoint;
        Point2f* originRightBrowPointPtr = &originRightBrowPoint;

        auto detectRightEyeBrow = [
                                  // output
                                  rightMatPtr
                                  ,originRightBrowPointPtr
                                  // input
                                  ,points
                                  ,cosa
                                  ,dst
            ...
            // for right eyebrow
            std::vector<Point2f> polygon = Feature::calculateBrowPolygon(points, true);
            ...
            int bottom = cvRound(points[33].y - rect_with_margin.y);
...
// need to move X coordinate with respect to the 1/slant.
            ...
            *originRightBrowPointPtr = origin;
            *rightMatPtr = affined_brow;

        };


        Mat leftMat; // = Mat();
        Mat* leftMatPtr = &leftMat;

        Point2f originLeftBrowPoint;
        Point2f* originLeftBrowPointPtr = &originLeftBrowPoint;

        Point2f left_makeup_center(static_cast<float>(makeup_moment.m10 / makeup_moment.m00),
                                   static_cast<float>(makeup_moment.m01 / makeup_moment.m00));

        Point2f* left_makeup_center_ptr = &left_makeup_center;

        auto detectLeftEyeBrow = [
                // output
                leftMatPtr
                ,originLeftBrowPointPtr
                // input
                ,points
                ,cosa
                ,dst
            ...
            // for left eyebrow
            std::vector<Point2f> polygon = Feature::calculateBrowPolygon(points, false);
            Moments moment = cv::moments(polygon);
            const Point2f center(static_cast<float>(moment.m10 / moment.m00),
                                 static_cast<float>(moment.m01 / moment.m00));
...
#if(USE_INPAINTING == 0)
            // TODO related to feature points, better move it to Feature class.
            int bottom = cvRound(points[32].y - rect_with_margin.y);
            if (bottom > target_mask.rows - 1)
                bottom = target_mask.rows - 1;
...
//		cv::imshow("target_mask", target_mask);
    ...
    //			*reinterpret_cast<cv::Vec3b*>(&dst_color) = src_color;
            ...
            // mirror image for left side
            cv::flip(makeup_mask, makeup_mask, 1/* horizontal */);
            (*left_makeup_center_ptr).x = makeup_rect.width - (*left_makeup_center_ptr).x;

            Vec2f scale(static_cast<float>(rect.width) / makeup_rect.width,
                        static_cast<float>(rect.height) / makeup_rect.height);

            Size target_size = makeup_rect.size();
            Point2f target_center = *left_makeup_center_ptr;
            Mat affine = Region::transform(target_size, target_center, angle, scale);
            cv::Mat affined_mask;
            cv::warpAffine(makeup_mask, affined_mask, affine, target_size, cv::INTER_LINEAR,
                           cv::BORDER_CONSTANT);
...
// need to move X coordinate with respect to the 1/slant.
            ...
            *originLeftBrowPointPtr = origin;
            *leftMatPtr = affined_brow;

        };


        std::thread leftTh(detectLeftEyeBrow);

        std::thread rightTh(detectRightEyeBrow);

        leftTh.join();
        //after leftThread
        Makeup::blend(dst, dst, *leftMatPtr, *originLeftBrowPointPtr, amount);

        rightTh.join();
        //after rightThread
        Makeup::blend(dst, dst, *rightMatPtr, *originRightBrowPointPtr, amount);

        uint64 end = GetTimeMs64();

        uint64 time_spent = end - begin;
        __android_log_print(ANDROID_LOG_VERBOSE, "NativeTime", "EyeBrow : %lu",
                            time_spent);
    ...
    void Makeup::applyEye(cv::Mat &dst, const cv::Mat &src, const std::vector<cv::Point2f> &points,
        ...
        */
        ...
        cv::Mat leftCos ; //= cv::Mat();
        cv::Mat* leftCosPtr = &leftCos;

        Point2i leftOrigin ; //= Point2i();
        Point2i* leftOriginPtr = &leftOrigin;

        auto detectLeftEyeThread = [
                                    //outputs
                                    leftCosPtr,
                                    leftOriginPtr,
                                    //inputs
            ...
            *leftCosPtr = _cosmetic;
            *leftOriginPtr = origin;


            // after leftThread
            // Makeup::blend(dst, dst, *leftCosPtr, *leftOriginPtr, amount);
        ...
        cv::Mat rightCos ; //= cv::Mat();
        cv::Mat* rightCosPtr = &rightCos;

        Point2i rightOrigin ; //= Point2i();
        Point2i* rightOriginPtr = &rightOrigin;

        auto detectRightEyeThread = [
                                    //outputs
                                    rightCosPtr,
                                    rightOriginPtr,
                                    //inputs
            ...
            *rightCosPtr = _cosmetic;
            *rightOriginPtr = origin;

            // after rightThread
            // Makeup::blend(dst, dst, *rightCosPtr, *rightOriginPtr, amount);

        };


        std::thread leftTh(detectLeftEyeThread);

        std::thread rightTh(detectRightEyeThread);
        ...
        uint64 time_spent = end - begin;
        __android_log_print(ANDROID_LOG_VERBOSE, "NativeTime", "applyEye : %lu",
                            time_spent);
    ...
    cv::Mat Makeup::createEyeShadow(cv::Mat mask[3], uint32_t color[3]/*, const int& COUNT = 3 */) {
        ...
        auto unpack = [](uint32_t c) -> cv::Vec3i {
            return cv::Vec3i(c & 0xFF, (c >> 8) & 0xFF,
                             (c >> 16) &
                             0xFF/*, (c>>24) & 0xFF*/);
        };
...
//	cv::imwrite("color2alpha.png", mask2);
        ...
        Point2i rightIrisPoint;
        Point2i* rightPointIrisPtr = & rightIrisPoint;

        Region rightRegion;
        Region* rightIrisRegionPtr = &rightRegion;

        Mat rightIris;
        Mat* rightIrisPtr = &rightIris;

        auto rightIrisThread = [
                          // input
                          points
                          ,mask_radius
                          ,amount
            ...
            const std::pair<cv::Point2f, float> iris_info = Feature::calculateIrisInfo(points,true);
            ...
            Region region = feature.calculateEyeRegion(true);

            *rightIrisPtr = iris;
            *rightIrisRegionPtr = region;
            *rightPointIrisPtr = origin;

        };

        Point2i leftIrisPoint;
        Point2i* leftPointIrisPtr = & leftIrisPoint;

        Region leftRegion;
        Region* leftRegionPtr = &leftRegion;

        Mat leftIrisMat;
        Mat* leftIrisMatPtr = &leftIrisMat;
        ...
        auto leftIrisThread = [
            ...
            const std::pair<cv::Point2f, float> iris_info = Feature::calculateIrisInfo(points,false);
            const cv::Point2f &center = iris_info.first;
            const float &radius = iris_info.second;
            ...
            Point2i origin = center - Point2f(iris.cols, iris.rows) / 2;
            Region region = feature.calculateEyeRegion(false);

            *leftIrisMatPtr = iris;
            *leftRegionPtr = region;
            *leftPointIrisPtr = origin;

        };

        std::thread leftTh(leftIrisThread);

        std::thread rightTh(rightIrisThread);

        leftTh.join();
        //after leftThread
        Makeup::blend(dst, dst, *leftIrisMatPtr, (*leftRegionPtr).mask, *leftPointIrisPtr, 1.0F);

        rightTh.join();
        //after rightThread
        Makeup::blend(dst, dst, *rightIrisPtr, (*rightIrisRegionPtr).mask, *rightPointIrisPtr, 1.0F);

        uint64 end = GetTimeMs64();

        uint64 time_spent = end - begin;
        __android_log_print(ANDROID_LOG_VERBOSE, "NativeTime", "Iris : %lu",
                            time_spent);
    ...
    Makeup::applyBlush(cv::Mat &dst, const cv::Mat &src, const std::vector<cv::Point2f> &points,
        ...
        Mat rightBlush;
        Mat* rightBlushPtr = &rightBlush;

        Rect rightRect;
        Rect* rightRectPtr = &rightRect;

        bool breakLeftThread = false;
        bool* breaker = &breakLeftThread;

        auto rightBlushThread = [
                            //input
                            points
                            ,shape
                            ,color
            ...
            std::vector<Point2f> polygon = createPolygon(points, shape, true);
            ...
            // cv::imshow(std::string("blush mask ") + (true), mask);

            *rightBlushPtr = blush;
            *rightRectPtr = rect;

            *breaker = shape == BlushShape::SEAGULL;

        };


        Mat leftBlush;
        Mat* leftBlushPtr = &leftBlush;

        Rect leftRect;
        Rect* leftRectPtr = &leftRect;


        auto leftBlushThread = [
                            //input
                            points
                            ,shape
                            ,color
            ...
            std::vector<Point2f> polygon = createPolygon(points, shape, false);
            Rect rect = cv::boundingRect(polygon);
            Mat mask = Feature::maskPolygonSmooth(rect, polygon,8);  // level (here 8) can be tuned.

            Mat blush = pack(mask, color);
		    //cv::imshow(std::string("blush mask ") + (false), mask);
        ...
        if(!*breaker){
            ...
            std::thread rightTh(rightBlushThread);
            std::thread leftTh(leftBlushThread);

            rightTh.join();
            // rightBlush
            blend(dst, dst, *rightBlushPtr, (*rightRectPtr).tl(), amount);

            leftTh.join();
            // leftBlush
            blend(dst, dst, *leftBlushPtr, (*leftRectPtr).tl(), amount);
        ...
        uint64 time_spent = end - begin;
        __android_log_print(ANDROID_LOG_VERBOSE, "NativeTime", "Blush : %lu",
                            time_spent);
