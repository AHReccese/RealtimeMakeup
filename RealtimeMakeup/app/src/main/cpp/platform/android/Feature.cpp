...
jobjectArray JNICALL Java_com_wonderful_ishow_makeup_Feature_nativeDetectFace(JNIEnv *env,
...
jobjectArray JNICALL Java_com_wonderful_ishow_makeup_Feature_nativeDetectFaces(JNIEnv *env,
		jclass clazz, jobject _image, jstring _image_name, jstring _classifier_dir)
{
	AndroidBitmapInfo image_info;
	uint32_t *pixels = lockJavaBitmap(env, _image, image_info);
...
jobjectArray JNICALL Java_com_wonderful_ishow_makeup_Feature_nativeGetSymmetryAxis(JNIEnv *env,
		jclass clazz, jobjectArray _points)
{

	const std::vector<Point2f> points = getNativePointArray(env, _points); // copy
